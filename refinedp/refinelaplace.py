...
def evaluate_refine_laplace(output_folder='./figures/refinelap'):
    ...
    plt.plot(x, pdf, label='\\Huge Laplace($\\mu$=0, scale=1)', linewidth=5)
    ...
    plt.plot(x, refined_pdf, label='\\Huge Laplace($\\mu$=0, scale={})'.format(refined_scale), linewidth=5)
    plt.title('\\Huge RefineLap (X, 1, 2)')
    plt.xticks(fontsize=26)
    plt.yticks(fontsize=26)
    plt.legend()
